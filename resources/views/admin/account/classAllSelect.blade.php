...
<br>
  <a href="{{ route('admin.account.class.user.assign.report.generate',Crypt::encrypt($user_id)) }}" class="btn btn-primary btn-sm pull-right" style="margin: 10px" target="blank">PDF Report</a>
<table class="table  table-bordered table-striped" id="class_section_list">
	...
	<tbody>
		@foreach ($userClassTypes as $userClassType)
					<tr>
						<td>{{ $userClassType->class_name}}</td> 
					</tr> 
		@endforeach
	</tbody>
</table> 
