              ...
              <div class="col-lg-6" style="margin-left: -40px">
                <ul class="sortable-posts">
                   @foreach($menuTypes as $menuType)
                    <ol class="ui-state-default" style="padding: 4px;font-size:20px" onclick="callAjax(this,'{{ route('admin.account.menu.filte',Crypt::encrypt($menuType->id)) }}','sub_menu_table')" id="{{ $menuType->id }}">{{ $menuType->sorting_id+1 }}<span class="ui-icon ui-icon-arrowthick-2-n-s"></span>{{ $menuType->name }}</ol>
                  @endforeach
                </ul>
              </div>
