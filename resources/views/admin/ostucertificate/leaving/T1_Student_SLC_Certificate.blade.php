 ...
 <body>
  ...
  <div align="center">
    ...
    UDISE Code : <b>{{ $SLCIssueDetails->udise_code }}</b> Department School Code : <b>{{ $SLCIssueDetails->department_school_code }}</b></P>
     
 <div align="center">
    <h3><u><b>SCHOOL LEAVING CERTIFICATE</b></u></h3>
    <P style="padding-left: 27px;"><h5>(Academic Year:  <b>{{ $SLCIssueDetails->academicYear->name or ''}}</b> )</h5></P>
  </div> 
  <div style="padding-top: 2px; padding-left: 13px;">
    <p>File No.<b>{{ $SLCIssueDetails->file_no }}</b><span style="float: right">Date of issue <b>{{ date('d-m-Y',strtotime($SLCIssueDetails->created_at)) }}</b></span></p>
  </div>
    <div style="padding-top: 2px; padding-left: 13px;"> 
